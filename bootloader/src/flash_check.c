...
#if defined(MCUBOOT_SLOT0_EXPECTED_WRITE_SIZE) ||                              \
    defined(MCUBOOT_SLOT1_EXPECTED_WRITE_SIZE)
BOOT_LOG_MODULE_DECLARE(mcuboot);

bool
swap_write_block_size_check(struct boot_loader_state *state)
{
    ...
    if (flash_write_block_size_pri != MCUBOOT_SLOT0_EXPECTED_WRITE_SIZE) {
        BOOT_LOG_DBG(
            "Discrepancy, slot0 expected write block size: %d, actual: %d",
    ...
    if (flash_write_block_size_sec != MCUBOOT_SLOT1_EXPECTED_WRITE_SIZE) {
        BOOT_LOG_DBG(
            "Discrepancy, slot1 expected write block size: %d, actual: %d",
