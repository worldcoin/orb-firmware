...
#define FLASH_AREA_IMAGE_PRIMARY(x)                                            \
    (((x) == 0) ? FIXED_PARTITION_ID(slot0_partition)                          \
                : FIXED_PARTITION_ID(slot0_partition))
#define FLASH_AREA_IMAGE_SECONDARY(x)                                          \
    (((x) == 0) ? FIXED_PARTITION_ID(slot1_partition)                          \
...
#define FLASH_AREA_IMAGE_PRIMARY(x)                                            \
    (((x) == 0)   ? FIXED_PARTITION_ID(slot0_partition)                        \
     : ((x) == 1) ? FIXED_PARTITION_ID(slot2_partition)                        \
                  : 255)
#define FLASH_AREA_IMAGE_SECONDARY(x)                                          \
    (((x) == 0)   ? FIXED_PARTITION_ID(slot1_partition)                        \
     : ((x) == 1) ? FIXED_PARTITION_ID(slot3_partition)                        \
                  : 255)
#else
