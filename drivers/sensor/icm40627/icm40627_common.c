...
LOG_MODULE_REGISTER(ICM40627_LL, CONFIG_SENSOR_LOG_LEVEL);
...
icm40627_reset(const struct device *dev)
{
    int res;
    uint8_t value;
    const struct icm40627_dev_cfg *dev_cfg = dev->config;

    /* start up time for register read/write after POR is 1ms and supply ramp
     * time is 3ms */
    k_msleep(3);

    /* perform a soft reset to ensure a clean slate, reset bit will auto-clear
     */
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_DEVICE_CONFIG,
                                    BIT_SOFT_RESET);
    ...
    res = icm40627_i2c_read(&dev_cfg->i2c, REG_INT_STATUS, &value, 1);
    ...
    res = icm40627_i2c_read(&dev_cfg->i2c, REG_WHO_AM_I, &value, 1);
    if (res) {
        return res;
    }

    if (value != WHO_AM_I_ICM40627) {
        LOG_ERR("invalid WHO_AM_I value, was %i but expected %i", value,
                WHO_AM_I_ICM40627);
        return -EINVAL;
    }

    /* Always use internal RC oscillator */
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_INTF_CONFIG1,
    ...
    return icm40627_i2c_single_write(&dev_cfg->i2c, REG_PWR_MGMT0, BIT_IDLE);
}

int
icm40627_configure(const struct device *dev, struct icm40627_cfg *cfg)
{
    struct icm40627_dev_data *dev_data = dev->data;
    const struct icm40627_dev_cfg *dev_cfg = dev->config;
    int res;

    /* Disable interrupts, reconfigured at end */
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_INT_SOURCE0, 0);

    /* if fifo is enabled right now, disable and flush */
    if (dev_data->cfg.fifo_en) {
        res = icm40627_i2c_single_write(
            &dev_cfg->i2c, REG_FIFO_CONFIG,
        ...
        res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_SIGNAL_PATH_RESET,
    ...
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_PWR_MGMT0, pwr_mgmt0);
    ...
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_ACCEL_CONFIG0,
                                    accel_config0);
    if (res != 0) {
    ...
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_GYRO_CONFIG0,
                                    gyro_config0);
    if (res != 0) {
    ...
    uint8_t fifo_config_bypass =
        FIELD_PREP(MASK_FIFO_MODE, BIT_FIFO_MODE_BYPASS);

    LOG_DBG("FIFO_CONFIG (0x%x) 0x%x", REG_FIFO_CONFIG, fifo_config_bypass);
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_FIFO_CONFIG,
                                    fifo_config_bypass);
    if (res != 0) {
        LOG_ERR("Error writing FIFO_CONFIG");
        return -EINVAL;
    ...
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_FSYNC_CONFIG, 0);
    if (res != 0) {
        LOG_ERR("Error writing FSYNC_CONFIG");
        return -EINVAL;
    }
    res = icm40627_i2c_read(&dev_cfg->i2c, REG_TMST_CONFIG, &tmst_config, 1);
    if (res != 0) {
        LOG_ERR("Error reading TMST_CONFIG");
        return -EINVAL;
    }
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_TMST_CONFIG,
                                    tmst_config & ~BIT(1));
    if (res != 0) {
    ...
    res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_INT_CONFIG,
    ...
    if ((cfg->accel_odr <= ICM40627_ACCEL_ODR_4000 ||
         cfg->gyro_odr <= ICM40627_GYRO_ODR_4000)) {
        int_config1 = FIELD_PREP(BIT_INT_TPULSE_DURATION, 1) |
                      FIELD_PREP(BIT_INT_TDEASSERT_DISABLE, 1);
    }

    res =
        icm40627_i2c_single_write(&dev_cfg->i2c, REG_INT_CONFIG1, int_config1);
    if (res) {
    ...
    if (cfg->fifo_en) {
         ...
         */
        uint8_t fifo_cfg1 = FIELD_PREP(BIT_FIFO_TEMP_EN, 1) |
        ...
        res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_FIFO_CONFIG1,
                                        fifo_cfg1);
        if (res != 0) {
        ...
        LOG_DBG("FIFO_CONFIG2( (0x%x)) (WM Low) 0x%x", REG_FIFO_CONFIG2,
                fifo_wml);
        res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_FIFO_CONFIG2,
                                        fifo_wml);
        if (res != 0) {
        ...
        LOG_DBG("FIFO_CONFIG3 (0x%x) (WM High) 0x%x", REG_FIFO_CONFIG3,
                fifo_wmh);
        res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_FIFO_CONFIG3,
                                        fifo_wmh);
        if (res != 0) {
        ...
        res = icm40627_i2c_single_write(&dev_cfg->i2c, REG_INT_SOURCE0,
                                        int_source0);
        if (res) {
            return res;
        }
...
{
    struct icm40627_dev_data *drv_data = dev->data;
    int ret = icm40627_configure(dev, cfg);

    if (ret == 0) {
        drv_data->cfg = *cfg;
    } else {
        ret = icm40627_configure(dev, &drv_data->cfg);
    }
...
icm40627_read_all(const struct device *dev, uint8_t data[14])
{
    ...
    res = icm40627_i2c_read(&dev_cfg->i2c, REG_TEMP_DATA1, data, 14);
