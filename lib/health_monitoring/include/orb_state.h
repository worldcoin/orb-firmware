 ...
 * diagnostics. The states are registered in a section, and can be iterated
 * over. Each state has a name, a status, and a message. The status is an
 * integer, and the message is a string. The message can be updated at runtime.
 *
 * Architecture has been inspired by the Zephyr's logging subsystem.
 */

#define ORB_STATE_MESSAGE_MAX_LENGTH 36 // 40 characters + null terminator
...
#define _ORB_STATE_CONST_DATA_CREATE(_name)                                    \
    ...
    BUILD_ASSERT((sizeof(STRINGIFY(_name)) < 12),                              \
