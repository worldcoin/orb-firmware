...
optics_self_test(void)
{
    ...
    for (size_t i = 0;
        ...
        if (pvcc_available) {
            ...
            LOG_ERR("%s didn't disable PVCC via eye safety circuitry",
                    ir_leds_names[i]);
            ORB_STATE_SET(RET_ERROR_UNSAFE,
                          "%s didn't disable PVCC via eye safety circuitry",
                          ir_leds_names[i]);
        } else {
    ...
    ORB_STATE_SET(RET_ERROR_NOT_SUPPORTED,
                  "safety self-test not implemented for diamond");
    return RET_SUCCESS;
#endif
}
                                         ...
                                         bool *triggered)
{
    ...
    if (pvcc_enabled) {
        ret = ORB_STATE_SET(RET_SUCCESS, "ir leds are usable (pvcc enabled)");
        ASSERT_SOFT(ret);
    } else {
        ret = ORB_STATE_SET(RET_ERROR_UNSAFE,
                            "ir leds are not usable (pvcc disabled)");
        ASSERT_SOFT(ret);
    }
