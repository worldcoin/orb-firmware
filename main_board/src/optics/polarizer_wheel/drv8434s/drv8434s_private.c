...
{
    switch (address) {
    case DRV8434S_REG_CTRL1_ADDR:
    case DRV8434S_REG_CTRL2_ADDR:
    case DRV8434S_REG_CTRL3_ADDR:
    case DRV8434S_REG_CTRL4_ADDR:
    case DRV8434S_REG_CTRL5_ADDR:
    case DRV8434S_REG_CTRL6_ADDR:
    case DRV8434S_REG_CTRL7_ADDR:
        return true;
        break;
    case DRV8434S_REG_FAULT_ADDR:
    case DRV8434S_REG_DIAGSTATUS1_ADDR:
    case DRV8434S_REG_DIAGSTATUS2_ADDR:
    case DRV8434S_REG_CTRL8_ADDR:
    case DRV8434S_REG_CTRL9_ADDR:
        if (write) {
            return false;
        } else {
            return true;
...
drv8434s_private_reg_read(uint8_t address, DRV8434S_Instance_t *instance)
{
    ...
    tx_word =
        tx_word | ((uint16_t)(address) << DRV8434S_SPI_TX_ADDRESS_START_POS);

    // Load r/w bit into tx word
    tx_word = tx_word | ((uint16_t)(DRV8434S_SPI_TX_RW_BIT_READ)
                         << DRV8434S_SPI_TX_RW_START_POS);
    ...
    instance->spi.tx_buffer[0u] = (tx_word >> 8) & DRV8434S_SPI_TX_LSB_MASK;
    instance->spi.tx_buffer[1u] = tx_word & DRV8434S_SPI_TX_LSB_MASK;
