...
#define POLARIZER_STEP_CHANNEL DT_PROP_BY_IDX(DT_NODELABEL(polarizer_step), channels, 0)

K_THREAD_STACK_DEFINE(stack_area_polarizer_wheel_home, THREAD_STACK_SIZE_POLARIZER_WHEEL_HOME);


static Polarizer_Wheel_Instance_t g_polarizer_wheel_instance;

// Peripherals (Motor Driver SPI, motor driver enable, encoder enable, encoder feedback, step PWM)
static const struct device *polarizer_spi_bus_controller = DEVICE_DT_GET(DT_NODELABEL(polarizer_controller));
static const struct pwm_dt_spec polarizer_step_pwm_spec = PWM_DT_SPEC_GET(DT_PATH(polarizer_step));
static const struct gpio_dt_spec polarizer_enable_spec = GPIO_DT_SPEC_GET(DT_PATH(zephyr_user), polarizer_stepper_enable_gpios);
static const struct gpio_dt_spec polarizer_encoder_enable_spec = GPIO_DT_SPEC_GET(DT_PATH(zephyr_user), polarizer_stepper_encoder_enable_gpios);
static const struct gpio_dt_spec polarizer_encoder_spec = GPIO_DT_SPEC_GET(DT_PATH(zephyr_user), polarizer_stepper_encoder_gpios);
static const TIM_TypeDef *polarizer_step_timer = ((TIM_TypeDef *)DT_REG_ADDR(DT_PARENT(DT_NODELABEL(polarizer_step))));
...
static ret_code_t enable_encoder_interrupt(void)
{
    return gpio_pin_interrupt_configure_dt(&polarizer_enable_spec, GPIO_INT_EDGE_RISING);
}

// Disable the interrupt
static ret_code_t disable_encoder_interrupt(void)
{
    return gpio_pin_interrupt_configure_dt(&polarizer_enable_spec, GPIO_INT_DISABLE);
}

// Enable polarizer wheel step interrupt
static ret_code_t enable_step_interrupt(void)
{
...
static ret_code_t disable_step_interrupt(void)
{
    polarizer_step_timer->DIER &= ~TIM_DIER_CC2IE;
...
static ret_code_t clear_step_interrupt(void)
{
...
static bool encoder_interrupt_enabled = false;

// Define callback function
static void encoder_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    ...
    if (pins & BIT(polarizer_encoder_spec.pin)) {
        if (gpio_pin_get_dt(&polarizer_encoder_spec) == 1) {
            if (!g_polarizer_wheel_instance.auto_homing.start_notch_found) {
                ...
                g_polarizer_wheel_instance.auto_homing.steps_to_last_notch = atomic_get(&g_polarizer_wheel_instance.step_count.step_count_current);
            }
            // Stop the PWM
...
static void polarizer_wheel_step_isr(void *arg)
{
    ARG_UNUSED(arg);

    atomic_inc(&g_polarizer_wheel_instance.step_count.step_count_current);
    uint32_t current = atomic_get(&g_polarizer_wheel_instance.step_count.step_count_current);
    if (current >= g_polarizer_wheel_instance.step_count.step_count_target) {
        pwm_set_dt(&polarizer_step_pwm_spec, 0, 0);
        g_polarizer_wheel_instance.movement.wheel_moving = false;
        atomic_set(&g_polarizer_wheel_instance.step_count.step_count_current, 0);
        g_polarizer_wheel_instance.step_count.step_count_target = 0;
        if (g_polarizer_wheel_instance.movement.target_position != POLARIZER_WHEEL_POSITION_UNKNOWN) {
            g_polarizer_wheel_instance.movement.current_position = g_polarizer_wheel_instance.movement.target_position;
            g_polarizer_wheel_instance.movement.target_position = POLARIZER_WHEEL_POSITION_UNKNOWN;
        }
        disable_step_interrupt();
    }
    clear_step_interrupt();
...
polarizer_wheel_init(void)
{
    ...
    ret_val = gpio_pin_configure_dt(&polarizer_enable_spec,
    ...
    // Configure the encoder input pin as an input and set up callback, keep interrupt disabled
    ret_val = gpio_pin_configure_dt(&polarizer_encoder_spec, GPIO_INPUT);
    if (ret_val != 0) {
        return RET_ERROR_INTERNAL;
    }

    gpio_init_callback(&polarizer_encoder_cb_data, encoder_callback, BIT(polarizer_encoder_spec.pin));
    ret_val = gpio_add_callback(polarizer_encoder_spec.port, &polarizer_encoder_cb_data);
    if (ret_val != 0) {
        return RET_ERROR_INTERNAL;
    }
    ...
    DRV8434_DriverCfg_t drv8434_cfg = {.spi_cfg =  {.frequency = 1000000,
...
polarizer_wheel_configure(void)
{
    ...
    DRV8434_DeviceCfg_t drv8434_cfg = { .ctrl2.EN_OUT = DRV8434_REG_CTRL2_VAL_ENOUT_DISABLE,
    ...
    timer2_register_callback(POLARIZER_STEP_CHANNEL, polarizer_wheel_step_isr, NULL);

    k_tid_t tid =
        k_thread_create(&thread_data_motor_phi, stack_area_polarizer_wheel_home,
                        K_THREAD_STACK_SIZEOF(stack_area_polarizer_wheel_home),
                        (k_thread_entry_t)polarizer_wheel_auto_homing_thread,
                        NULL, NULL, NULL,
...
static ret_code_t start_polarizer_wheel_step(uint32_t frequency, uint32_t step_count)
{
    ...
    // Start PWM
    ret_val = pwm_set_dt(&polarizer_pwm, period_ns, pulse_ns);
...
static void polarizer_wheel_auto_homing_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);

    g_polarizer_wheel_instance.auto_homing.auto_homing_state = POLARIZER_WHEEL_AUTO_HOMING_STATE_IN_PROGRESS;
    ...
    while ((g_polarizer_wheel_instance.auto_homing.notches_encountered <= POLARIZER_WHEEL_ENCODER_MAX_HOMING_NOTCHES)
           && (!g_polarizer_wheel_instance.movement.wheel_moving)) {

        if (g_polarizer_wheel_instance.auto_homing.start_notch_found == true) {
            switch (g_polarizer_wheel_instance.auto_homing.notches_encountered) {
                case 0:
                    break;
                case 1:
                    last_step_count = g_polarizer_wheel_instance.auto_homing.steps_to_last_notch;
                    break;
                default:
                    if (abs(g_polarizer_wheel_instance.auto_homing.steps_to_last_notch - last_step_count) <= 100) {
                        g_polarizer_wheel_instance.auto_homing.auto_homing_state = POLARIZER_WHEEL_AUTO_HOMING_STATE_SUCCESS;
                        g_polarizer_wheel_instance.movement.current_position = POLARIZER_WHEEL_POSITION_PASS_THROUGH;
                    }
                    break;
            }
        }
        if ((g_polarizer_wheel_instance.auto_homing.auto_homing_state != POLARIZER_WHEEL_AUTO_HOMING_STATE_SUCCESS)
            && (g_polarizer_wheel_instance.auto_homing.notches_encountered < POLARIZER_WHEEL_ENCODER_MAX_HOMING_NOTCHES)) {
            ...
            if (g_polarizer_wheel_instance.auto_homing.auto_homing_state != POLARIZER_WHEEL_AUTO_HOMING_STATE_SUCCESS) {
                g_polarizer_wheel_instance.auto_homing.auto_homing_state = POLARIZER_WHEEL_AUTO_HOMING_STATE_FAILED;
            }
            break;
        }
    }
...
Polarizer_Wheel_Position_t polarizer_wheel_get_position(void)
{
...
polarizer_wheel_set_position(Polarizer_Wheel_Position_t position)
{
    ...
    if (ret_val != RET_SUCCESS) {
        ...
        g_polarizer_wheel_instance.movement.current_position = POLARIZER_WHEEL_POSITION_UNKNOWN;
    }
    return ret_val;
}
