...
#endif
static enum boot_progress_step_e boot_progress = BOOT_PROGRESS_STEP_UNKNOWN;
static uint32_t pulsing_index = 0;

#define BOOT_PROGRESS_PERCENT_FULL 100
#define BOOT_PROGRESS_PERCENTAGE_STEP                                          \
    (BOOT_PROGRESS_PERCENT_FULL / (BOOT_PROGRESS_STEP_FULL_COUNT))
...
{
    if (step > BOOT_PROGRESS_STEP_FULL_COUNT) {
        // log issue
...
{
    if (boot_progress != BOOT_PROGRESS_STEP_FULL_COUNT) {
        return RET_SUCCESS;
    }
