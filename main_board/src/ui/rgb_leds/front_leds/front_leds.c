...
#endif
static uint32_t boot_progress_percentage = 0; // from 0 to 100
static uint32_t pulsing_index = 0;

#define BOOT_PROGRESS_PERCENT_FULL    100
#define BOOT_PROGRESS_PERCENTAGE_STEP 20 // ⚠️ depends on number of calls
...
{
    // should be called only up to 100%
    if (boot_progress_percentage + BOOT_PROGRESS_PERCENTAGE_STEP > 100) {
        // log issue
    ...
    pulsing_index = 0;
    boot_progress_percentage =
        MIN(100, boot_progress_percentage + BOOT_PROGRESS_PERCENTAGE_STEP);
}
    ...
    for (;;) {
        ...
        if (!use_sequence) {
            ...
            case orb_mcu_main_UserLEDsPattern_UserRgbLedPattern_BOOT_ANIMATION:
                ...
                scaler = MAX(0.0f, scaler - 0.3f); // cut off lower brightness
                color.r = roundf(scaler * color.r);
                color.g = roundf(scaler * color.g);
                color.b = roundf(scaler * color.b);
                ...
                wait_until = K_MSEC(global_pulsing_delay_time_ms);
                set_center((struct led_rgb)RGB_OFF);
                uint32_t angle_progress =
                    boot_progress_percentage * FULL_RING_DEGREES / 100;
                // fill with solid color up to progress angle
                set_ring((struct led_rgb)global_color, &rgb_off, 90,
                         -angle_progress);
                // moving head
                uint32_t boot_anim_start_angle =
                    (uint32_t)(90 - (int32_t)angle_progress < 0
                                   ? 360 + (90 - (int32_t)angle_progress)
                                   : 90 - (int32_t)angle_progress);
...
{
    if (boot_progress_percentage < 100) {
        return RET_SUCCESS;
    }
