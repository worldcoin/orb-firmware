...
voltage_measurement_self_test_thread()
{
    ...
    uint32_t self_test_passed_ms = 0;
    ...
    while (self_test_passed_ms < VOLTAGES_SELF_TEST_SUSTAIN_PERIOD_MS &&
           timeout--) {
        ...
        for (size_t i = 0; i < CHANNEL_COUNT; i++) {
            ...
            if (voltage_measurement_tests[i].min != 0 &&
                voltage_measurement_tests[i].max != 0) {
                if (voltage_mv < voltage_measurement_tests[i].min ||
                    voltage_mv > voltage_measurement_tests[i].max) {
                    LOG_ERR("[ FAIL ] %s = %d mV; NOT in range: [%d, %d]",
                            voltage_measurement_channel_names[i], voltage_mv,
                            voltage_measurement_tests[i].min,
                            voltage_measurement_tests[i].max);
                    fail_count++;
                } else {
                    if (self_test_passed_ms == 0) {
                        LOG_INF(
                            "[  OK  ] %s = %d mV; min = %d mV; max = %d mV; in "
                            "range [%d, %d]",
                            voltage_measurement_channel_names[i], voltage_mv,
                ...
                if (self_test_passed_ms == 0) {
                    LOG_INF("[ SKIP ] %s = %d mV; min = %d mV; max = %d mV",
                            voltage_measurement_channel_names[i], voltage_mv,
                            min_voltage_mv, max_voltage_mv);
                }
            }
        ...
        if (fail_count == 0) {
            ...
            self_test_passed_ms += VOLTAGES_SELF_TEST_PERIOD_MS;
        } else {
            LOG_ERR("📈 Voltages not in range!");
            if (self_test_passed_ms >= VOLTAGES_SELF_TEST_PERIOD_MS) {
                self_test_passed_ms -= VOLTAGES_SELF_TEST_PERIOD_MS;
            } else {
                self_test_passed_ms = 0;
            }
        }
    ...
    } else if (self_test_passed_ms == VOLTAGES_SELF_TEST_SUSTAIN_PERIOD_MS) {
                         ...
                         struct k_mutex *analog_mux_mutex)
{
    ...
    k_tid_t tid_debug = k_thread_create(
        ...
        NULL, THREAD_PRIORITY_VOLTAGE_MEASUREMENT_DEBUG, 0, K_SECONDS(4));
