...
SFU_BOOT_Init(void);
static SFU_ErrorStatus
SFU_BOOT_DeInit(void);
static void
SFU_BOOT_BspConfiguration(void);
static SFU_ErrorStatus
SFU_BOOT_SM_Run(void);
static void
SFU_BOOT_SM_CheckStatusOnReset(void);
#if (SECBOOT_LOADER == SECBOOT_USE_LOCAL_LOADER) || (SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER)
static void
SFU_BOOT_SM_CheckNewFwToDownload(void);
static void
SFU_BOOT_SM_DownloadNewUserFw(void);
#endif /* (SECBOOT_LOADER == SECBOOT_USE_LOCAL_LOADER) || (SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER) */
#ifdef KMS_ENABLED
static void
SFU_BOOT_SM_CheckKMSBlobToInstall(void);
static void
SFU_BOOT_SM_InstallKMSBlob(void);
#endif /* KMS_ENABLED */
static void
SFU_BOOT_SM_CheckUserFwStatus(void);
static void
SFU_BOOT_SM_VerifyUserFwSignature(void);
static void
SFU_BOOT_SM_ExecuteUserFw(void);
static void
SFU_BOOT_SM_HandleCriticalFailure(void);
static void
SFU_BOOT_SM_RebootStateMachine(void);
static void
SFU_BOOT_SM_InstallNewUserFw(void);
static void
SFU_BOOT_SM_ResumeInstallNewUserFw(void);
static void
SFU_BOOT_SM_RollbackInstallPrevUserFw(void);
...
static SFU_ErrorStatus
SFU_BOOT_SystemSecurity_Config(void);
static SFU_ErrorStatus
SFU_BOOT_CheckApplySecurityProtections(void);
static SFU_ErrorStatus
SFU_BOOT_SecuritySafetyCheck(void);
static void
SFU_BOOT_ManageResetSources(void);
...
SFU_BOOT_SM_Run(void)
{
    ...
    void
    (*fnStateMachineFunction)(void);
    static void
    (*fnStateMachineTable[])(void) = {SFU_BOOT_SM_CheckStatusOnReset,
#if (SECBOOT_LOADER == SECBOOT_USE_LOCAL_LOADER) || (SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER)
...
//    }
    ...
    SFU_SET_SM_IF_CURR_STATE(e_ret_status,
                             SFU_STATE_DOWNLOAD_NEW_USER_FW,
                             SFU_STATE_CHECK_KMS_BLOB_TO_INSTALL);
#else
    /* Set the next State Machine state according to the success of the failure of e_ret_status */
...
SFU_BOOT_SM_CheckKMSBlobToInstall(void)
{
    ...
    /* Set the next State Machine state according to the success of the failure of e_ret_status */
    SFU_SET_SM_IF_CURR_STATE(e_ret_status,
                             SFU_STATE_INSTALL_KMS_BLOB,
                             SFU_STATE_VERIFY_USER_FW_STATUS);
}
...
SFU_BOOT_SM_CheckUserFwStatus(void)
{
    ...
    e_PendingInstallStatus =
        SFU_IMG_CheckPendingInstallation(&m_DwlSlotToInstall, &m_ActiveSlotToResume,
    ...
    switch (e_PendingInstallStatus)
    {
        case SFU_IMG_FWIMAGE_ROLLBACK:
            ...
            TRACE(
                ...
                m_ActiveSlotToRollback,
                m_DwlSlotToInstall - SLOT_DWL_1 + 1U);
        ...
        case SFU_IMG_FWIMAGE_TO_INSTALL:
             ...
             */
            TRACE("\r\n\t  New Fw to be installed from slot SLOT_DWL_%d",
                  m_DwlSlotToInstall - SLOT_DWL_1 + 1U);
        ...
        case SFU_IMG_NO_FWUPDATE:
            ...
            if (MASTER_SLOT != 0xFFU)
            {
                if (SFU_SUCCESS == SFU_IMG_DetectFW(MASTER_SLOT))
                {
                    ...
                    TRACE("\r\n\t  A FW is detected in the slot SLOT_ACTIVE_%d",
                    ...
                    if (SlotStartAdd[SLOT_ACTIVE_1 + i] != 0U)       /* Slot configured ? */
                    {
                        if (SFU_IMG_VerifyEmptyActiveSlot(SLOT_ACTIVE_1 + i) != SFU_SUCCESS)
                        {
                            ...
                            (void) SFU_IMG_InvalidateCurrentFirmware(
                                SLOT_ACTIVE_1 + i); /* If this fails we continue anyhow */
                        }
                    }
                }
...
SFU_BOOT_SM_DownloadNewUserFw(void)
{
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
        ...
        /* Read header in dwl slot */
        e_ret_status = SFU_LL_FLASH_Read((uint8_t *) &fw_image_header_validated,
                                         (uint8_t *) SlotStartAdd[dwl_slot],
                                         sizeof(SE_FwRawHeaderTypeDef));
    }
    ...
    else
    {
        ...
        {
            case SFU_LOADER_ERR_COM:SFU_EXCPT_SetError(SFU_EXCPT_COM_ERR);
                break;
            case SFU_LOADER_ERR_FW_VERSION:SFU_EXCPT_SetError(SFU_EXCPT_VERSION_ERR);
                break;
            case SFU_LOADER_ERR_FW_LENGTH:SFU_EXCPT_SetError(SFU_EXCPT_FW_TOO_BIG);
                break;
            case SFU_LOADER_ERR_AUTH_FAILED:SFU_EXCPT_SetError(SFU_EXCPT_HEADER_AUTH_ERR);
                break;
            case SFU_LOADER_ERR_FLASH:SFU_EXCPT_SetError(SFU_EXCPT_FLASH_ERR);
                break;
            case SFU_LOADER_ERR_CRYPTO:SFU_EXCPT_SetError(SFU_EXCPT_DECRYPT_ERR);
                break;
            default:
    ...
    /* Set the next State Machine state according to the success of the failure of e_ret_status */
    SFU_SET_SM_IF_CURR_STATE(e_ret_status,
                             SFU_STATE_REBOOT_STATE_MACHINE,
                             SFU_STATE_HANDLE_CRITICAL_FAILURE);
}
#endif /* SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER */
...
SFU_BOOT_SM_HandleCriticalFailure(void)
{
    ...
    SFU_SET_SM_IF_CURR_STATE(SFU_ERROR,
                             SFU_STATE_REBOOT_STATE_MACHINE,
                             SFU_STATE_REBOOT_STATE_MACHINE);
}
...
SFU_BOOT_SystemSecurity_Config(void)
{
    ...
    if (SFU_BOOT_CheckApplySecurityProtections() != SFU_SUCCESS)
    {
        /* WARNING: This might be generated by an attempted attack or a bug of your code!
           ...
           ...
        */
        TRACE("\r\n= [SBOOT] System Security Check failed: 0x! Rebooting...");
    }
    else
    {
...
/**
  ...
  */
void
SFU_CALLBACK_ANTITAMPER(RTC_HandleTypeDef *hrtc)
{
    UNUSED(hrtc);
