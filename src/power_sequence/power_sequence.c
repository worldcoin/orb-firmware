...
int turn_on_power_supplies(void)
{
    ...
    if (gpio_pin_configure(supply_1v8_pg, SUPPLY_1V8_PG_PIN, SUPPLY_1V8_PG_FLAGS | GPIO_INPUT)) {
...
int turn_on_jetson(void)
{
    ...
    const struct device *lte_gps_usb_reset = DEVICE_DT_GET(LTE_GPS_USB_RESET_CTLR);
    ...
    if (gpio_pin_configure(sleep_wake, SLEEP_WAKE_PIN, SLEEP_WAKE_FLAGS | GPIO_OUTPUT)) {
        LOG_ERR("Error configuring sleep wake pin!");
        return 1;
    }

    if (gpio_pin_configure(power_enable, POWER_ENABLE_PIN, POWER_ENABLE_FLAGS | GPIO_OUTPUT)) {
        LOG_ERR("Error configuring power enable pin!");
        return 1;
    }

    if (gpio_pin_configure(system_reset, SYSTEM_RESET_PIN, SYSTEM_RESET_FLAGS | GPIO_INPUT)) {
        LOG_ERR("Error configuring system reset pin!");
        return 1;
    }

    if (gpio_pin_configure(lte_gps_usb_reset, LTE_GPS_USB_RESET_PIN, LTE_GPS_USB_RESET_FLAGS | GPIO_OUTPUT)) {
