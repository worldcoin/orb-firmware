...
const struct device *front_unit_temp_sensor =
    DEVICE_DT_GET(DT_NODELABEL(front_unit_tmp_sensor));
const struct device *mcu_internal_sensor = DEVICE_DT_GET(DT_PATH(stm_tmp));
...
static void
sample_front_unit_temperature(void)
{
    int ret;
    int32_t temp;

    ret = get_ambient_temperature(front_unit_temp_sensor, &temp,
                                  SENSOR_CHAN_AMBIENT_TEMP);
    if (!ret) {
        msg.message.m_message.payload.temperature.source =
            Temperature_TemperatureSource_FRONT_UNIT;
...
static void
sample_mcu_internal_temperature(void)
{
    int ret;
    int32_t temp;

    ret = get_ambient_temperature(mcu_internal_sensor, &temp,
...
thread_entry_point(void *a, void *b, void *c)
{
    ...
    for (;;) {
        ...
        sample_front_unit_temperature();
        sample_mcu_internal_temperature();
    }
}
...
{
    if (!device_is_ready(front_unit_temp_sensor)) {
        LOG_ERR("Could not initialize front unit temp sensor!");
        return;
    }
    if (!device_is_ready(mcu_internal_sensor)) {
        LOG_ERR("Could not initialize mcu internal temp sensor!");
        return;
    }
